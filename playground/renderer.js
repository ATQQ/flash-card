...
async function createCardRenderer(canvas, assets) {
    ...
    ['character', 'background', 'ui', 'structure'].map(
      (name) =>
        new Promise((resolve, reject) => {
          ...
          if (asset instanceof HTMLCanvasElement) return resolve(asset);
  ...
  return {
    draw(x, y, strength, depth = 0, contourBrightness = 0.15, adjust = {}) {
      use(scene);
