---
name: holo-card-lite
description: Create interactive holographic cards from supplied images with colored layers, repaired scenery, signed parallax and contour glow. Fork of holo-card; outputs a lightweight HTML that references external assets/ by default (~30KB), with an optional --inline self-contained mode. Use Codex native image tools with local assembly, or an explicitly selected independent API workflow.
---
...
3. Generate separate **colored** layers with the built-in image tool:
   - **Character:** continuous colored foreground, preserving clothing, anatomy and dark details. Remove overlaid text and repair the concealed artwork coherently. Use a regular neutral checkerboard matte outside the silhouette; convert it locally to an alpha mask before final import. If the source window is a baked transparency checkerboard, that checkerboard is background, never silhouette: the mask must be the figure itself (plus genuine overflow beyond the window), not the window rectangle.
   - **Background:** identify scenery and reconstruct every area hidden by subjects, typography and frame. Produce a complete opaque plate, without holes, glyphs or frame fragments, with room for moving crops. The renderer magnifies the plate's central 50% by exactly 2× (`b=(p-.5)*.5+.5-view*.25`), so compose the plate zoomed-out: the content that should fill the art window is drawn at half size, centered, with usable scenery margin around it — a plate painted at final scale displays twice too large and the motif spills past the window.
   - **UI:** combine original text, numbers, symbols, panels and the entire decorative frame at one depth. Preserve glyphs, colors and layout. Use the same checkerboard-matte workflow outside actual UI shapes. The renderer lays the background plate under the entire card and ignores its alpha, so any transparent margin in the UI plate would leak the background around the frame; `assemble` handles this automatically — it clips the composite card rectangle to the UI plate's opaque bounding box (inset 2px past the a==255 bounds to eat anti-aliased matte residue) and passes it to the renderer via `HOLO_CARD_RECT`. Do not inpaint or extend pixels into the margin.
...
- Background: remove the main figure and overlays and reconstruct the concealed scene. Never use the original image enlarged, blurred or dimmed as the completed plate; it creates a second figure under parallax. A fully opaque file alone does not prove successful inpainting. Verify the central-50%-at-2× mapping: the motif visible through the window must read at its designed size, not magnified, and the plate must keep scenery margin around the window area for parallax shifts.
- UI: keep text, panels, decorative frame and editorial inset portraits together when appropriate. Preserve their original pixels/layout where possible. Do not transfer inset faces or poster text into the main-character layer. The plate must be opaque to all four canvas edges (rounded corners are clipped by the renderer); sample the border band and reject any transparent or semi-transparent margin, or the background bleeds around the frame.
...
- Automated pre-render checks worth scripting: (1) the composite card rect equals the UI plate's opaque bounding box (inset ~2px), so no transparent margin survives inside it; (2) the only large transparent region in the UI plate is the art window; (3) background plate fully opaque with its motif sized for the 2× center crop; (4) character alpha over the window matches the figure silhouette, never a filled rectangle.
...
Publish the updated HTML containing the QR entry to that same URL. In the default external mode the QR is referenced as `mobile-qr.png`, so upload it next to the HTML. This argument generates a link and QR only; it does not upload or establish availability. Preserve the target access policy and state if the link requires login. Do not claim real phone verification from synthetic orientation events.
