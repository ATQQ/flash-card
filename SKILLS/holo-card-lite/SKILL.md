...
Because the default HTML references `assets/*.png` and `assets/back.png`, always serve the whole job folder (index.html + assets/), never a lone index.html. If a preview fails to load images, the assets were not served beside the HTML, not a renderer issue.
