...
def html_document(job,state,inline=False):
    """Default: HTML references external assets/ files (~30KB; keep index.html beside its job folder).
    inline=True produces a self-contained HTML with base64-embedded images (10MB+, one-file sharing only)."""
    ...
    manifest={'name':state['name'],'width':state['source']['width'],'height':state['source']['height'],
              'assets':{k:data_url(job/'assets'/f'{k}.png') if inline else f'assets/{k}.png' for k in KINDS},
              'back':data_url(job/'assets/back.png') if inline else 'assets/back.png'}
...
def assemble(job,preview_url=None,inline=False):
    state=load(job)
    ...
    names += [f'assets/{k}.png' for k in KINDS]+['assets/back.png']
    names += [f'masks/{k}.png' for k in KINDS if (job/'masks'/f'{k}.png').exists()]
...
def main():
    ...
    for command in ('assemble','status'):
        ...
        if command=='assemble':
            p.add_argument('--preview-url');p.add_argument('--inline',action='store_true',help='Emit a self-contained HTML with base64-embedded images (large); default references external assets/')
    ...
    if args.command=='prepare':result=prepare(Path(args.source).expanduser().resolve(),Path(args.output).expanduser().resolve(),args.name)
        ...
        elif args.command=='assemble':result=assemble(job,args.preview_url,args.inline)
